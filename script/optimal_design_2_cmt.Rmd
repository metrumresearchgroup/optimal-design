...
A wealth of data in adults has allowed us to describe orally-administered mockdrozaline PK using a 2-compartment model with first-order absorption, and 2 covariates: weight on clearance (`wt_cl`, shared by `CL` and `Q`) and weight on volume (`wt_v`, shared by `V2` and `V3`).

```{r echo=FALSE}
tibble(
  CL = 10, V2 = 100, Q = 1, V3 = 30, KA = 0.25,
  wt_cl = 0.75, wt_v = 1
) %>% 
  knitr::kable()
```
...
```{r echo=FALSE}
tibble(
  om_CL = 0.08, om_V2 = 0.1, sigma = 0.05
) %>% 
  knitr::kable()
```
...
In this example, we include IIV on `CL` and `V2`, and pass through dose, tau, and body weight as covariates.
...
This is clearly not an informative design.  After some behind-the-scenes experimentation, it turns out that if we fix `Q` and `V3` we start to get more reasonable estimates.
...
We can live with fixing those parameters.  The RSEs are looking pretty good, but let's see if we can make any improvements with optimization.

# *D*-optimal design

## Starting from the original design

```{r optimal,cache=TRUE,message=FALSE,results='hide'}
output <- poped_optim(
  poped_db_fix_qv3,
  opt_xt = TRUE,
  parallel = TRUE,
...
Add another post dose sample between 1 and 6 hours post dose at steady state.
...
```{r}
poped_db_pd_ss <- create.poped.database(
  ff_fun = ff,
  fg_fun = fg,
  fError_fun = feps,
  bpop = c(CL = 10, V2 = 100, Q = 1, V3 = 30, KA = 0.25), 
  ...
  m = 1,
  groupsize = 12,
  xt = c(5, c(2, rep(24, 6), 168) + 24),
  minxt = c(1, c(1, rep(24, 6), 168) + 24 - 1),
  maxxt = c(6, c(6, rep(24, 6), 168) + 24),
  bUseGrouped_xt = 0,
  a = cbind(DOSE = 10, TAU = 24, WT = 32)
)
```
...
```{r}
FIM_pd_ss <- evaluate.fim(poped_db_pd_ss) 
get_rse(FIM_pd_ss, poped_db_pd_ss)
```

```{r optimal_pd_ss,cache=TRUE,message=FALSE,results='hide'}
output_pd_ss <- poped_optim(
  poped_db_pd_ss,
  opt_xt = TRUE,
  parallel = TRUE,
  parallel_type = "multicore",
...
saveRDS(output_pd_ss, "opt2.rds")
```

```{r}
output_pd_ss <- readRDS("opt2.rds")
summary(output_pd_ss)
```
...
We didn't have a whole lot of wiggle room in the design space, so our design (and RSEs) haven't changed much.  The Day 1 sample got pushed to the maximum allowable (6 hours), and the trough samples got pushed back to the earliest allowable (1 hour before dosing).
...
* up to 2 hours before the first sample at 6 hours (i.e., 4-6 hours post dose)
